...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <title>{{ $title ?? 'Tococo Careers — Join the Coconut Revolution' }}</title>
        
        <!-- Primary Meta Tags -->
        <meta name="title" content="{{ $title ?? 'Tococo Careers — Join the Coconut Revolution' }}">
        <meta name="description" content="{{ $metaDescription ?? 'Build your career with Tococo Indonesia. Join an ecosystem dedicated to innovation, local empowerment, and sustainable agriculture.' }}">
        ...
        <meta property="og:title" content="{{ $title ?? 'Tococo Careers — Join the Coconut Revolution' }}">
        <meta property="og:description" content="{{ $metaDescription ?? 'Build your career with an ecosystem dedicated to innovation.' }}">
    ...
    <body class="antialiased bg-brand-white text-brand-charcoal selection:bg-brand-emerald/10 selection:text-brand-emerald" x-data="{ mobileMenuOpen: false }">
        ...
        <nav class="fixed w-full z-100 transition-all duration-300 bg-white/95 backdrop-blur-md border-b border-gray-100">
            <div class="container-tococo">
                <div class="flex justify-between items-center h-20">
                    ...
                    <div class="hidden lg:block">
                        <a href="#" class="px-6 py-2.5 rounded-full bg-brand-emerald text-white text-xs font-black uppercase tracking-widest hover:bg-brand-forest transition-all shadow-lg shadow-brand-emerald/20">
                            Talent Portal
                        </a>
                    </div>
            ...
            <div x-show="mobileMenuOpen" 
                ...
                <a href="{{ route('career.index') }}" class="block golden-caption text-brand-emerald">Career</a>
                <a href="{{ route('comunity.index') }}" class="block golden-caption">Community</a>
            </div>
        </nav>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-golden-md md:gap-golden-base">
                    <div class="md:col-span-5">
                        ...
                        <p class="golden-body mb-golden-md">
                            Build your future with a zero-waste ecosystem dedicated to innovation and local empowerment.
                        </p>
                    ...
                    <div class="md:col-span-4 flex flex-col items-start md:items-end text-left md:text-right">
                        <h3 class="golden-caption text-brand-emerald mb-golden-base block opacity-80">Join Us</h3>
                        <div class="space-y-1">
                            <a href="mailto:hr@tococoindonesia.com" class="block text-sm font-bold text-brand-emerald hover:text-brand-forest transition-all">
                                hr@tococoindonesia.com
                            </a>
                        </div>
                    </div>
                </div>
