...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <title>{{ $title ?? 'Tococo News — Latest Updates & Insights' }}</title>
        
        <!-- Primary Meta Tags -->
        <meta name="title" content="{{ $title ?? 'Tococo News — Latest Updates & Insights' }}">
        <meta name="description" content="{{ $metaDescription ?? 'Stay updated with the latest news from Tococo Indonesia. From sustainable agriculture to coconut innovation.' }}">
        ...
        <meta property="og:title" content="{{ $title ?? 'Tococo News — Latest Updates & Insights' }}">
        <meta property="og:description" content="{{ $metaDescription ?? 'Stay updated with the latest news from Tococo Indonesia.' }}">
    ...
    <body class="antialiased bg-brand-white text-brand-charcoal selection:bg-brand-emerald/10 selection:text-brand-emerald" x-data="{ mobileMenuOpen: false }">
        ...
        <nav class="fixed w-full z-50 transition-all duration-300 bg-white/95 backdrop-blur-md border-b border-gray-100">
            ...
            <div x-show="mobileMenuOpen" 
                ...
                <a href="{{ route('landing.home') }}" class="block golden-caption hover:text-brand-emerald">Home</a>
                <a href="{{ route('landing.about') }}" class="block golden-caption hover:text-brand-emerald">About</a>
                <a href="{{ route('landing.products') }}" class="block golden-caption hover:text-brand-emerald">Products</a>
                <a href="{{ route('news.index') }}" class="block golden-caption text-brand-emerald">News</a>
                <a href="{{ route('career.index') }}" class="block golden-caption hover:text-brand-emerald">Career</a>
                <a href="{{ route('comunity.index') }}" class="block golden-caption hover:text-brand-emerald">Community</a>
            </div>
        </nav>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-golden-md md:gap-golden-base">
                    <div class="md:col-span-5">
                        ...
                        <p class="golden-body mb-golden-md">
                            Sharing the latest developments and insights from the heart of Indonesian coconut innovation.
                        </p>
                    ...
                    <div class="md:col-span-4 flex flex-col items-start md:items-end text-left md:text-right">
                        ...
                        <div class="space-y-1">
                            <a href="mailto:news@tococoindonesia.com" class="block text-sm font-bold text-brand-emerald hover:text-brand-forest transition-all">
                                news@tococoindonesia.com
                            </a>
                        </div>
                    </div>
                </div>
